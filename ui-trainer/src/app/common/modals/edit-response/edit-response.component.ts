...
export class EditResponseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.editResponseForm = new FormGroup({
      responseDisplay: new FormControl({value: this.data.responseDisplay, disabled: true}, Validators.required),
  ...
  closeDialog() {
    if (this.editResponseForm.valid) {
      this.dialogRef.close({
        ...
        response_display: this.data.editResponseForm.value.responseDisplay,
