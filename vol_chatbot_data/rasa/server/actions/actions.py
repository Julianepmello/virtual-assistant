...
class ActionChangeField(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        ## Mod_direto é um vetor de validação para entrar no modo de criação de documento direto
        if ent == None:
            dispatcher.utter_message("Esse campo não está disponível para alteração")
            return[]            
...
class InformContact(FormAction):
    ...
    def required_slots(self, tracker: Tracker) -> List[Text]:
        return["user_name", "user_email", "user_number_contact", "confirm_message",
                "user_message"]
    ...
    def slot_mappings(self) -> Dict[Text, Union[Dict, List[Dict]]]:
        return {
            ...
            "user_number_contact": [self.from_entity(entity="number_contact"),
                               self.from_text(not_intent = ["cancel", "change"])],
            "confirm_message": [self.from_intent(intent = ['affirm', 'acknowledge'], value = True),
    ...
    def validate_user_number_contact (self,value,dispatcher,tracker:Tracker, domain):
    ...
    def validate_confirm_message (self,value,dispatcher,tracker:Tracker, domain):
        if value == True:
            ...
            dispatcher.utter_message("Desculpa, mas não entendi. Vou te perguntar de novo")
            return{"confirm_message": None}
    
    def validate_user_message (self,value,dispatcher,tracker:Tracker, domain):
...
class ActionRestart(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        return[Restarted()] #Reiniciar a conversa
...
class ActionFillSlotCanceled(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_message("Pronto, cancelei o envio. Você quer saber mais alguma coisa sobre a Kyros?")
